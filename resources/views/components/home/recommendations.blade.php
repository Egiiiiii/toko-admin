        ...
        <div class="flex gap-6 overflow-x-auto pb-8 pt-2 px-1 snap-x snap-mandatory no-scrollbar">
            ...
            <div class="flex-none w-72 snap-start">
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 hover:shadow-2xl transition-all duration-300 flex flex-col h-full group hover:-translate-y-2">
                    <a href="{{ route('product.show', $product) }}" class="relative aspect-square overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 rounded-t-2xl block">
                        @if($product->image)
                    ...
                    <div class="p-5 flex-grow flex flex-col">
                        <span class="text-xs font-semibold text-blue-600 mb-2">{{ $product->category->name ?? 'Umum' }}</span>
                        <a href="{{ route('product.show', $product) }}" class="text-lg font-bold text-gray-900 line-clamp-2 mb-3 hover:text-blue-600">
                            {{ $product->name }}
                        </a>
                        ...
                        <div class="mt-auto">
                            <div class="flex items-end justify-between">
                                <div>
                                    <div class="text-sm text-gray-400 line-through">Rp {{ number_format($product->price * 1.1, 0, ',', '.') }}</div>
                                    <div class="text-2xl font-bold text-gray-900">Rp {{ number_format($product->price, 0, ',', '.') }}</div>
                                </div>
                                <a href="{{ route('product.show', $product) }}" class="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-600/30">
